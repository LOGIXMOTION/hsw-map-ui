...
class DrawingManager {
    ...
    if (this.assetsData[zone]) {
      poopi = `<div class="zone-box" id="Embedded" style="background-color:${this.polyColor}">
                <div class="zone-header">
                    <div class="zone-name">${zone} (${this.assetsData[zone].length})</div>
  ...
  setupMarkerFormHandling(marker, markerType) {
    ...
    popup.on('add', () => {
      ...
      if (coordsInput) {
        coordsInput.value = `${currentLatLng.lat.toFixed(15)}, ${currentLatLng.lng.toFixed(15)}`;
      }
      ...
      marker.off('drag dragend').on('drag dragend', () => {
        ...
        if (currentPopup && currentPopup.isOpen()) {
          ...
          if (coordsInput) {
            coordsInput.value = `${newLatLng.lat.toFixed(15)}, ${newLatLng.lng.toFixed(15)}`;
          }
        }
      });
    });
